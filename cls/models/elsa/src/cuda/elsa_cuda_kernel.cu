...
template <typename scalar_t>
__global__ void ELSAForward(const int num_kernels, const scalar_t *__restrict__ bottom_data,
                            const scalar_t *__restrict__ bottom_channel_mul,
                            const scalar_t *__restrict__ bottom_channel_add,
        ...
        for (iy = start_h; iy < end_h; iy+=dilation) {
            ...
            for (ix = start_w; ix < end_w; ix+=dilation) {
                ...
                input = bottom_data[bottom_id] *
                        ...
                        bottom_channel_mul[channel_filter_id] + bottom_channel_add[channel_filter_id]);
...
int ELSAForwardLauncher(const at::Tensor features, const at::Tensor channel_mul,
                        const at::Tensor channel_add,
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
        features.type(), "ELSAForward", ([&] {
            ...
            const scalar_t *bottom_channel_mul = channel_mul.data<scalar_t>();
            const scalar_t *bottom_channel_add = channel_add.data<scalar_t>();
            const scalar_t *bottom_spatial_filter = spatial_filter.data<scalar_t>();
            scalar_t *top_data = output.data<scalar_t>();
            ELSAForward<scalar_t>
                <<<grid, THREADS_PER_BLOCK, 0, stream>>>(
                num_kernels, bottom_data, bottom_channel_mul, bottom_channel_add,
...
template <typename scalar_t>
__global__ void ELSABackward_Feature(const int num_kernels, const scalar_t *__restrict__ top_diff,
                                     const scalar_t *__restrict__ bottom_spatial_filter,
                                     const scalar_t *__restrict__ bottom_channel_mul,
                                     const scalar_t *__restrict__ bottom_channel_add,
                                     const int kernel_size, const int dilation,
        ...
        for (bottom_iy = start_h; bottom_iy < end_h; bottom_iy+=dilation){
            ...
            for (bottom_ix = start_w; bottom_ix < end_w; bottom_ix+=dilation){
                ...
                input = (shared_spatial_filter[spatial_filter_c * WARP_SIZE + pixel_id]
                    * bottom_channel_mul[channel_filter_id]
                    + bottom_channel_add[channel_filter_id])
                    * top_diff[top_id];
                t = output_val + input;
                lost += fabs(output_val) >= fabs(input) ? (output_val - t) + input
                                                        : (input - t) + output_val;
...
template <typename scalar_t>
__global__ void ELSABackward_Spatial_Filter(const int num_kernels,
    ...
    for (c = lane_id; c < channels; c += WARP_SIZE) {
        ...
        input = top_diff[top_id] * bottom_data[bottom_id] * bottom_channel_mul[channel_filter_id];
        t = output_val + input;
        lost += fabs(output_val) >= fabs(input) ? (output_val - t) + input
                                              : (input - t) + output_val;
        output_val = t;
...
template <typename scalar_t>
__global__ void ELSABackward_Channel_Filter(const int num_kernels,
                                            const scalar_t *__restrict__ top_diff,
                                            const scalar_t *__restrict__ bottom_data,
                                            const scalar_t *__restrict__ bottom_spatial_filter,
                                            const int kernel_size, const int dilation,
                                            ...
                                            const int top_height, const int top_width,
                                            const int bottom_height, const int bottom_width,
                                            scalar_t *__restrict__ channel_mul_diff,
                                            scalar_t *__restrict__ channel_add_diff){
    int index = threadIdx.x + blockIdx.x * blockDim.x;
    if (index > num_kernels - 1) {
        return;
    }
    ...
    scalar_t output_val_mul = 0;
    scalar_t lost_mul = 0;
    scalar_t t_mul = 0;
    scalar_t input_mul = 0;

    scalar_t output_val_add = 0;
    scalar_t lost_add = 0;
    scalar_t t_add = 0;
    scalar_t input_add = 0;
    ...
    for (index = lane_id; index < top_height*top_width; index+=WARP_SIZE){
        ...
        input_mul = top_diff[top_id] * bottom_data[bottom_id] * bottom_spatial_filter[spatial_filter_id];
        t_mul = output_val_mul + input_mul;
        lost_mul += fabs(output_val_mul) >= fabs(input_mul)
            ? (output_val_mul - t_mul) + input_mul
            : (input_mul - t_mul) + output_val_mul;
        output_val_mul = t_mul;

        input_add = top_diff[top_id] * bottom_data[bottom_id];
        t_add = output_val_add + input_add;
        lost_add += fabs(output_val_add) >= fabs(input_add)
            ? (output_val_add - t_add) + input_add
            : (input_add - t_add) + output_val_add;
        output_val_add = t_add;
    }
    __syncwarp();
    output_val_mul = warpReduceSum(output_val_mul);
    lost_mul = warpReduceSum(lost_mul);

    output_val_add = warpReduceSum(output_val_add);
    lost_add = warpReduceSum(lost_add);
    if (lane_id == 0) {
        const int channel_filter_id = Loc2Index(n, c, kernel_iy, kernel_ix,
            channels, kernel_size, kernel_size);
        channel_mul_diff[channel_filter_id] = output_val_mul + lost_mul;
        channel_add_diff[channel_filter_id] = output_val_add + lost_add;
    }
}


int ELSABackwardLauncher(const at::Tensor top_grad, const at::Tensor features,
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
        top_grad.type(), "ELSABackward_Feature", ([&] {
    ...
    AT_DISPATCH_FLOATING_TYPES(
        top_grad.type(), "ELSABackward_Spatial_Filter", ([&] {
            ...
            ELSABackward_Spatial_Filter<scalar_t>
                <<<at::cuda::ATenCeilDiv(num_kernels, THREADS_PER_BLOCK),
                    THREADS_PER_BLOCK, 0, stream>>>(
                    num_kernels, top_diff, bottom_data, bottom_channel_mul,
    ...
    AT_DISPATCH_FLOATING_TYPES(
        top_grad.type(), "ELSABackward_Channel_Filter", ([&] {
            const int num_kernels = batch_size * channels *
                kernel_size * kernel_size * WARP_SIZE;
            const scalar_t *top_diff = top_grad.data<scalar_t>();
            const scalar_t *bottom_data = features.data<scalar_t>();
            const scalar_t *bottom_spatial_filter = spatial_filter.data<scalar_t>();
            scalar_t *channel_mul_diff = channel_mul_grad.data<scalar_t>();
            scalar_t *channel_add_diff = channel_add_grad.data<scalar_t>();

            ELSABackward_Channel_Filter<scalar_t>
                <<<at::cuda::ATenCeilDiv(num_kernels, THREADS_PER_BLOCK),
                    THREADS_PER_BLOCK, 0, stream>>>(
                    num_kernels, top_diff, bottom_data, bottom_spatial_filter,
                    kernel_size, dilation, stride, channels, top_height,
                    top_width, bottom_height, bottom_width,
                    channel_mul_diff, channel_add_diff);
    }));
