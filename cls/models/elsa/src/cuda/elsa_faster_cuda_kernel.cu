...
#define MAX_KS 5
#define DATA_TILE 14
#define CHANNEL_THREADS 5
#define CHANNEL_BLOCKS 1
...
template <typename scalar_t>
__global__ void ELSAForward(const scalar_t *__restrict__ bottom_data,
    ...
    __shared__ scalar_t shared_channel_mul[CHANNEL_THREADS * MAX_KS * MAX_KS];
    __shared__ scalar_t shared_channel_add[CHANNEL_THREADS * MAX_KS * MAX_KS];
    ...
    for (int c = cb_id * CHANNEL_THREADS  + threadIdx.z; c < channels; c += CHANNEL_BLOCKS * CHANNEL_THREADS) {
        ...
        if (threadIdx.x < kernel_size && threadIdx.y < kernel_size){
            ...
            shared_channel_mul[(threadIdx.z * kernel_size + threadIdx.y) * kernel_size + threadIdx.x] =
                bottom_channel_mul[channel_filter_id];
            shared_channel_add[(threadIdx.z * kernel_size + threadIdx.y) * kernel_size + threadIdx.x] =
            ...
            for (int iy = start_y; iy < end_y; iy+=dilation) {
                ...
                for (int ix = start_x; ix < end_x; ix+=dilation) {
                    ...
                    input = shared_data[(threadIdx.z * DATA_TILE + iy) * DATA_TILE + ix] *
                        ...
                        shared_channel_mul[threadIdx.z * kernel_size * kernel_size + filter_c] +
                        shared_channel_add[threadIdx.z * kernel_size * kernel_size + filter_c]);
...
int ELSAFasterForwardLauncher(const at::Tensor features, const at::Tensor channel_mul,
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
        features.type(), "ELSAForward", ([&] {
            ...
            ELSAForward<scalar_t><<<grid, block, 0, stream>>>(
                bottom_data, bottom_channel_mul, bottom_channel_add,
                bottom_spatial_filter,
